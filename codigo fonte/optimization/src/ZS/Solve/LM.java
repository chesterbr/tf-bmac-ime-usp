...
// see comment above
import otimizacao.MetodosDeGradiente;
import Jama.*;
...
public final class LM {
	...
	static class LMQuadTest implements LMfunc {
		 ...
		 * 
		 * z = (s*(px-ox))^2 + (s*(py-oy))^2 dz/dox = -2(s*(px-ox))*s dz/ds =
		 * 2*s*[(px-ox)^2 + (py-oy)^2]
		 * 
	...
	static class LMGaussTest implements LMfunc {
		...
		/**
		 * <pre>
		 * y(x) = \sum B_k exp(-((x - E_k) / G_k)&circ;2)
		 * arg  =  (x-E_k)/G_k
		 * ex   =  exp(-arg*arg)
		 * fac =   B_k * ex * 2 * arg
		 * 
		 * d/dB_k = exp(-((x - E_k) / G_k)&circ;2)
		 * d/dE_k = B_k exp(-((x - E_k) / G_k)&circ;2) . -2((x - E_k) / G_k) . -1/G_k
		 *        = 2 * B_k * ex * arg / G_k
		 *   d/E_k[-((x - E_k) / G_k)&circ;2] = -2((x - E_k) / G_k) d/dE_k[(x-E_k)/G_k]
		 *   d/dE_k[(x-E_k)/G_k] = -1/G_k
		 * d/G_k = B_k exp(-((x - E_k) / G_k)&circ;2) . -2((x - E_k) / G_k) . -(x-E_k)/G_k&circ;2
		 *       = B_k ex -2 arg -arg / G_k
		 *       = fac arg / G_k
		 *   d/dx[1/x] = d/dx[x&circ;-1] = -x[x&circ;-2]
		 * 
		 */
		public double grad(double[] x, double[] a, int a_k) {
			assert x.length == 1;
