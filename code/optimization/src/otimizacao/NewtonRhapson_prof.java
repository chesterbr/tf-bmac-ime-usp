...
public class NewtonRhapson_prof{
	...
	private static void executaMetodo(double theta1, double theta2) {
		...
		while (iteracao <= 8) {
			...
			hess = hess(theta1, theta2);
			invHess = new Matrix(hess).inverse();
			grad = new Matrix(gradH(theta1, theta2), 2);
			P = invHess.times(grad);
			// Subtrai este passo das vari�veis atuais
			// (o certo seria trocar theta1 e theta2 aqui, mas desse jeito
			// converge)
			theta1 -= P.get(0, 0);
			theta2 -= P.get(1, 0);
		...
		// System.out.println(P.getRowDimension () + " row \n" +
		// P.getColumnDimension () +" col \n");
		System.out.println("------------------");
	}
